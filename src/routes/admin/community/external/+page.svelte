...
<div class="mx-auto max-w-4xl p-6">
	<h1 class="mb-6 text-3xl font-bold text-orange-700">External Community Administration</h1>

	<div class="mb-8 grid gap-6">
		<section class="rounded-lg bg-white p-6 shadow">
			<h2 class="mb-4 text-xl font-semibold">Quick Actions</h2>
			<div class="grid grid-cols-3 gap-4">
				{#if hasPermission('external.information')}
					<a
						href="/admin/community/external/information"
				...
				{/if}
				{#if hasPermission('external.events')}
					<a
						href="/admin/community/external/events"
						class=" flex items-center justify-center rounded bg-orange-500 p-4 text-center text-white transition hover:bg-orange-600"
					>
						Manage Community Events
					</a>
				{/if}
				{#if hasPermission('external.workshops')}
					<a
						href="/admin/community/external/workshops"
						class=" flex items-center justify-center rounded bg-orange-500 p-4 text-center text-white transition hover:bg-orange-600"
					>
						Manage Workshops
					</a>
				...
				{#if hasPermission('external')}
					<a
						href="/admin/community/external/map"
					...
					>
						External Community At A Glance
					</a>
				{/if}
			</div>
		</section>
