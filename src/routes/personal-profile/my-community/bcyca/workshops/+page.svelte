<script lang="ts">
	...
	let communityWorkshopChoices = $state(
		data.userProfile?.community_bcyca_profile?.community_workshop_choices
	);
	let otherCommunityWorkshop = $state(
		data.userProfile?.community_bcyca_profile?.other_community_workshop
	);
	let willRunCommunityWorkshops = $state(
		data.userProfile?.community_bcyca_profile?.will_run_community_workshops
	);
	...
	function handleReset() {
		if (confirm('Are you sure you want to undo? All unsaved changes will be lost.')) {
			...
			communityWorkshopChoices =
				data.userProfile?.community_bcyca_profile?.community_workshop_choices;
			otherCommunityWorkshop = data.userProfile?.community_bcyca_profile?.other_community_workshop;
