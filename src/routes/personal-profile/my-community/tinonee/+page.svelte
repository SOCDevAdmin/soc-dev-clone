<script lang="ts">
	...
	let otherComments = $state(data.userProfile.community_tinonee_profile?.other_comments);
	let stayInTouchChoices = $state(
		data.userProfile.community_tinonee_profile?.stay_in_touch_choices
	);
	let userPostalAddress = $state(data.userProfile.user_postal_address);
	...
	function handleReset() {
		if (confirm('Are you sure you want to undo? All unsaved changes will be lost.')) {
			...
			otherComments = data.userProfile.community_tinonee_profile?.other_comments;
			stayInTouchChoices = data.userProfile.community_tinonee_profile?.stay_in_touch_choices;
