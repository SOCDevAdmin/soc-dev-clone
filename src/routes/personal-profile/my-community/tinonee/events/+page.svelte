<script lang="ts">
	...
	let communityMeetingChoices = $state(
		data.userProfile?.community_tinonee_profile?.community_meeting_choices
	);
	let otherCommunityMeeting = $state(
		data.userProfile?.community_tinonee_profile?.other_community_meeting
	);
	...
	function handleReset() {
		if (confirm('Are you sure you want to undo? All unsaved changes will be lost.')) {
			...
			communityMeetingChoices =
				data.userProfile?.community_tinonee_profile?.community_meeting_choices;
			otherCommunityMeeting = data.userProfile?.community_tinonee_profile?.other_community_meeting;
		}
	}
</script>
