<script lang="ts">
	...
	let informationSheetChoices = $state(
		data.userProfile?.community_tinonee_profile?.information_sheet_choices
	);
	let otherInformationSheet = $state(
		data.userProfile?.community_tinonee_profile?.other_information_sheet
	);
	...
	function handleReset() {
		if (confirm('Are you sure you want to undo? All unsaved changes will be lost.')) {
			...
			informationSheetChoices =
				data.userProfile?.community_tinonee_profile?.information_sheet_choices;
			otherInformationSheet = data.userProfile?.community_tinonee_profile?.other_information_sheet;
		}
	}
</script>
